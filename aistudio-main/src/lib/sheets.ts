...
function parseNoteRows(rows: any[][] | null | undefined): NoteItem[] {
    if (!rows || rows.length <= 1) {
        return [];
    }
    ...
    return rows.slice(1).map((row, index): NoteItem | null => {
        if (row.every(cell => !cell)) return null;
    ...
    }).filter((e): e is NoteItem => e !== null).sort((a, b) => new Date(b.updatedAt).getTime() - new Date(a.updatedAt).getTime());
}
...
export async function getNotes(): Promise<NoteItem[]> {
    try {
        ...
        await ensureSheetExists(sheets, sheetName, ['id', 'title', 'content', 'updatedAt']);
...
export async function addNote(noteData: Omit<NoteItem, 'id' | 'updatedAt'>): Promise<NoteItem> {
    const sheets = getSheets();
    const sheetName = 'Notes';
    await ensureSheetExists(sheets, sheetName, ['id', 'title', 'content', 'updatedAt']);
    ...
    const newRow = [newNote.id, newNote.title, newNote.content, newNote.updatedAt];
...
export async function updateNote(noteData: NoteItem): Promise<NoteItem> {
    const sheets = getSheets();
    const sheetName = 'Notes';
    const found = await findRowById(sheets, sheetName, noteData.id);

    if (found === null) {
        throw new Error('Note not found to update');
    }
    
    noteData.updatedAt = toZonedTime(new Date(), TIME_ZONE).toISOString();

    const { rowIndex } = found;
    const updatedRow = [noteData.id, noteData.title, noteData.content, noteData.updatedAt];
...
export async function deleteNote(id: string): Promise<void> {
    const sheets = getSheets();
    const sheetName = 'Notes';
    const found = await findRowById(sheets, sheetName, id);
